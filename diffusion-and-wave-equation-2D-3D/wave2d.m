...
% satisfies the wave equation in two dimensions. This solution requires 
...
if ndims(U_init) ~= 2 || ndims(dU_init) ~= 2
    ...
    for ix = 1:nx      % for each spatial value in the x direction
        for iy = 1:ny  % for each spatial value in the y direction
            if U_out(ix, iy, it) == -Inf       % if the current spatial value is unknown
                ...
                U_out(ix, iy, it) = 2*Utmp - U_out(ix, iy, it - 2); % use the value from the time just before, solve for the current value

                for dxy = [-1 1 0 0; 0 0 -1 1]  % using the points around it (i.e. up, down, left right)
                    dix = ix + dxy(1);
                    diy = iy + dxy(2);

                    if ~isnan( U_out(dix, diy, it - 1) ) % if it is not an insultaed boundary, solve using finite difference eqn
