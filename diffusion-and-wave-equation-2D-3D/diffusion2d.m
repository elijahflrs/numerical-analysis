% eeflores
% diffusion2d.m
% This function aims to find a function u(t, x, y) that 
% satisfies the diffusion equation in two dimensions. This solution requires 
...
%    U_out - A matrix of size size nx x ny x nt u values approximating the solution. 

function [t, U_out] = diffusion2d( kappa, h, U_init, U_bndry, t_int, nt )
...
if ndims(U_init) ~= 2
    throw(MException('MATLAB:invalid_argument', ...
        'The argument U_init or dU_init is not a 2D array.'));
end
...
U_out = zeros( nx, ny, nt );    % construct the solution matrix
U_out(:, :, 1) = U_init;        % include the initual values 

r = kappa*dt/h^2;               % ratio to check to ensure convergence

if r >= 0.25
    nt_suggest = ceil((kappa / (0.25*h^2)) * (tf-ti) + 1);
    warning( 'MATLAB:questionable_argument', ...
           'The coefficient r = kappa*dt/h^2 = %d >= 0.25. Consider using nt = %d', ...
            r, nt_suggest)
end
...
% Solving
% =======
for it = 2:nt      % for each time value
    U_out(:, :, it) = U_bndry( t(it), nx, ny );  % find and implement the boundary condition for t(it)
    
    for ix = 1:nx      % for each spatial value in the x direction
        for iy = 1:ny  % for each spatial value in the y direction
            if U_out(ix, iy, it) == -Inf       % if the current spatial value is unknown
                Utmp = U_out(ix, iy, it - 1);  
                U_out(ix, iy, it) = Utmp; % use the values from the time just before

                for dxy = [-1 1 0 0; 0 0 -1 1]  % using the points around it (i.e. up, down, left right)
                    dix = ix + dxy(1);
                    diy = iy + dxy(2);

                    if ~isnan( U_out(dix, diy, it - 1) ) % if it is not an insulated boundary, solve using finite difference eqn
